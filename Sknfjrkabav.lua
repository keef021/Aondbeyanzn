-- High Menu - Mini City RP (Versão Corrigida)
-- Fly com TweenService + Otimizações

-- Anti-Detecção Melhorada
local function improvedAntiDetection()
    spawn(function()
        while wait(3) do
            pcall(function()
                local rs = game:GetService("ReplicatedStorage")
                local suspicious = {"CheckAnti", "AntiCheat", "Security", "Detection"}
...
-- UI Library Carregamento Seguro
local Library
pcall(function()
...
-- Função de atualização de personagem melhorada
...
State.player.CharacterAdded:Connect(function()
    ...
    -- Reativar funcionalidades se necessário
...
InfoTab:AddParagraph({ "Versão:", "High Menu v2.0 Fixed" })
...
-- Stats atualizadas
local statsLabel = InfoTab:AddParagraph({ "Estatísticas:", "Tempo: 0s | FPS: 60" })
...
spawn(function()
    while wait(1) do
        ...
        statsLabel:Set("Estatísticas:", string.format(
            "Tempo: %ds | FPS: %d", 
            math.floor(elapsed), 
            fps
        ))
    ...
    if enabled then
        ...
        local function monitorHealth()
            ...
            if State.humanoid and autoCLActive then
                ...
                healthConnection = State.humanoid.HealthChanged:Connect(function(health)
                    if autoCLActive and State.humanoid then
                        ...
                        if healthPercentage <= 15 and health > 0 then
                            print("⚠️ Vida baixa detectada (" .. math.floor(healthPercentage) .. "%) - Ativando Auto CL...")
                ...
                -- Armazenar conexão para limpeza posterior
        ...
        -- Armazenar conexão para limpeza
        if not State.movement.connections.autoCL then
            State.movement.connections.autoCL = {}
        end
        table.insert(State.movement.connections.autoCL, charConnection)
        
        print("🔄 Auto CL ativado - Sair quando vida ≤ 15%")
    else
        -- Limpar todas as conexões do Auto CL
        ...
        print("🔄 Auto CL desativado")
    end
end

-- Sistema Auto CL (Quit quando vida baixa)
local autoCLActive = false
local healthThreshold = 20 -- Valor absoluto de vida crítica
...
local function setupAutoCL(enabled)
    ...
    if enabled then
                ...
                healthConnection = State.humanoid.HealthChanged:Connect(function(health)
                    if autoCLActive and State.humanoid and health > 0 then
                        ...
                        if health <= healthThreshold then
                            print("⚠️ VIDA CRÍTICA! (" .. math.floor(health) .. " HP) - SAINDO AGORA!")
                            healthConnection:Disconnect()
                            
                            -- Sair super rápido - sem delay
                            spawn(function()
                                State.player:Kick("Auto CL - Vida crítica! (" .. math.floor(health) .. " HP)")
                            end)
                        end
                    end
                end)
                
                -- Verificação inicial da vida atual
                if State.humanoid.Health <= healthThreshold and State.humanoid.Health > 0 then
                    print("⚠️ VIDA JÁ CRÍTICA! (" .. math.floor(State.humanoid.Health) .. " HP) - SAINDO AGORA!")
                    healthConnection:Disconnect()
                    spawn(function()
                        State.player:Kick("Auto CL - Vida crítica! (" .. math.floor(State.humanoid.Health) .. " HP)")
                    end)
                end
        ...
        local charConnection = State.player.CharacterAdded:Connect(function()
            if autoCLActive then
                wait(1) -- Aguardar apenas 1 segundo para ativar mais rápido
                monitorHealth()
            end
        end)
        ...
        print("🔄 Auto CL ativado - Sair quando vida ≤ " .. healthThreshold .. " HP")
    else
        -- Limpar todas as conexões do Auto CL
        if State.movement.connections.autoCL then
            for _, connection in pairs(State.movement.connections.autoCL) do
...
InfoTab:AddSlider({
    ...
    Callback = function(value)
        ...
        print("🩺 Limite de vida alterado para " .. value .. " HP")
    end
})
...
MovementTab:AddParagraph({
    "⚠️ AVISO IMPORTANTE:",
    "Use o pulo infinito com cuidado você pode acabar morrendo de dano de queda e use a velocidade de caminhada para ir mais rápido usando o pulo infinito"
})
...
FarmTab:AddSection({ "Sistema de Coleta Automática" })

-- Função melhorada para encontrar lixos
...
-- Função de movimento suave para farm
local function tweenToPosition(targetPosition, speed)
    ...
    duration = math.min(duration, 3) -- Máximo 3 segundos por movimento
    ...
    while not completed and State.farmActive and (tick() - startTime) < (duration + 1) do
        wait(0.05)
        -- Verificar se ainda está se movendo na direção certa
...
local function startTrashFarm()
    ...
    spawn(function()
        print("🗑️ Iniciando Farm Lixeiro...")
        ...
        while State.farmActive do
            ...
            if State.rootPart and State.humanoid then
                ...
                if #trashList > 0 then
                    local target = trashList[1] -- Pegar o mais próximo
        ...
        -- Desativar fly se não estava ativo antes
        if not wasFlying then
            activateTweenFly(false)
        end
        
        print("🛑 Farm Lixeiro parado!")
    end)
end

FarmTab:AddToggle({
    Name = "Farm Lixeiro Automático",
    Default = false,
    ...
    Callback = function()
        ...
        if #trashList > 0 then
            ...
            if State.rootPart then
                ...
                print("📍 Teleportado para área de lixos!")
            end
        else
...
ESPTab:AddToggle({
    ...
    Default = false,
    Callback = function(enabled)
        -- Implementar ESP para lixos se necessário
        if enabled then
            print("🗑️ Lixo ESP ativado!")
        else
            print("🗑️ Lixo ESP desativado!")
        end
    end
})
...
local function performCleanup()
    print("🧹 Realizando limpeza do sistema...")
...
print("✅ High Menu v2.0 carregado com sucesso!")
print("🚀 Fly com TweenService implementado!")
print("📈 Sistema otimizado e estável!")
